...
static s32 ov5640_write_reg(u16 reg, u8 val)
{
	...
	if (ret < 0) {
		pr_info("%s(mipi):reg=%x,val=%x error=%d\n",
			__func__, reg, val, ret);
		return ret;
	}
	pr_info("%s(mipi):reg=%x,val=%x\n", __func__, reg, val);
	return 0;
}
...
static s32 ov5640_read_reg(u16 reg, u8 *val)
{
	...
	if (ret < 0) {
		pr_err("%s(mipi):addr=%x reg=%x ret=%d\n", __func__, client->addr, reg, ret);
		return ret;
	}
	*val = buf[0];
	pr_debug("%s(mipi):addr=%x reg=%x,val=%x\n", __func__, client->addr, reg, buf[0]);
	return buf[0];
}
			    ...
			    enum ov5640_mode mode, enum ov5640_mode orig_mode)
{
	...
	pr_info("%s: framerate = %d\n", __func__,  ov5640_framerates[frame_rate] );
	...
	else if (ov5640_data.pix.pixelformat == V4L2_PIX_FMT_SRGGB8) {
	    mipi_csi2_set_datatype(mipi_csi2_info,MIPI_DT_RAW8);
	    pr_info("%s: set pixelformat V4L2_PIX_FMT_SRGGB8\n",__func__);
	}
	else if (ov5640_data.pix.pixelformat == V4L2_PIX_FMT_SRGGB10) {
	...
	if (mode == ov5640_mode_INIT) {
		...
		ov5640_data.pix.width = 1312;
		ov5640_data.pix.height = 978;
		//retval = ov5640_download_firmware(pModeSetting, ArySize);
		//if (retval < 0)
		//	goto err;
			...
			const struct i2c_device_id *id)
{
	...
	ov5640_data.pix.pixelformat = V4L2_PIX_FMT_SRGGB10;
	ov5640_data.pix.width = 1312;
	ov5640_data.pix.height = 978;
...
//	ov5640_standby(0);
	...
	imx224_read_reg(0x3000, &imx224data);
	imx224_read_reg(0x3380, &imx224data);
	imx224_read_reg(0x3381, &imx224data);
	imx224_read_reg(0x337D, &imx224data);
...
//	clk_disable_unprepare(ov5640_data.sensor_clk);
	...
	pr_info("camera imx224_mipi is found\n");
	return retval;
}
