...
//static int cam_sel2_gpio, cam_sel1_gpio;
...
void setup_ifparm(cam_data *cam, int init_defrect) 
{
	...
	pr_info("JOE'S FUNCTION GOT CALLED\n");
...
int mxc_cam_select_input(cam_data *cam, int index)
{
	...
	if (strcmp(mxc_capture_inputs[index].name, "CSI MEM") == 0) {
		...
		pr_info("%s:CSI IC MEM JOEV\n", __func__);
		retval = csi_enc_select(cam);//retval = prp_enc_select(cam);
#endif
	}
	if (retval) {
...
static int mxc_v4l_close(struct file *file)
{
	...
	if (--cam->open_count == 0) {
		...
		if (strcmp(mxc_capture_inputs[cam->current_input].name,
			...
			pr_info("%s:CSI MEM JOEV\n", __func__);
#endif
		} else if (strcmp(mxc_capture_inputs[cam->current_input].name,
				  "CSI IC MEM") == 0) {
#if defined(CONFIG_MXC_IPU_PRP_ENC) || defined(CONFIG_MXC_IPU_PRP_ENC_MODULE)
			err |= csi_enc_deselect(cam);//err |= prp_enc_deselect(cam);
			pr_info("%s:CSI IC MEM JOEV\n", __func__);
#endif
		}
			    ...
			    unsigned int ioctlnr, void *arg)
{
	...
	case VIDIOC_REQBUFS: {
		...
		if (!gpio_is_valid(cam_sel2_gpio)) {
			dev_warn(&cam->video_dev->dev, "no cam_sel2_gpio pin available");
			return -EINVAL;
		}
		gpio_free(cam_sel2_gpio);
		
		retval = devm_gpio_request_one(&cam->video_dev->dev, cam_sel2_gpio, GPIOF_OUT_INIT_LOW,
						"imx224_mipi_camsel2");
		if (retval < 0) {
			dev_warn(&cam->video_dev->dev, "request of cam_sel2_gpio failed");
			return retval;
		}
		...
		if (!gpio_is_valid(cam_sel1_gpio)) {
			dev_warn(&cam->video_dev->dev, "no cam_sel1_gpio pin available");
			return -EINVAL;
		}
		gpio_free(cam_sel1_gpio);
	
		
		retval = devm_gpio_request_one(&cam->video_dev->dev, cam_sel1_gpio, GPIOF_OUT_INIT_LOW,
						"imx224_mipi_camsel1");
		if (retval < 0) {
			dev_warn(&cam->video_dev->dev, "request of cam_sel1_gpio failed");
			return retval;
		}
		*/
